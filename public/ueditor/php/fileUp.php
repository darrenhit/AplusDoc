...
//上传配置
$config = array(
    "uploadPath" => "/home/pplive/gfs/file/" , //保存路径
...
if ( $state == "SUCCESS" ) {
    ...
    $result = move_uploaded_file( $clientFile[ "tmp_name" ] , $path . $fileName );
    if ( !$result ) {
        $state = "文件保存失败！";
    }
}
//向浏览器返回数据json数据
$filePath = "index/getFile?name=";
echo '{"state":"' . $state . '","url":"' . $filePath . $fileName . '","fileType":"' . $current_type . '","original":"'.$clientFile["name"] .'"}';
?>
